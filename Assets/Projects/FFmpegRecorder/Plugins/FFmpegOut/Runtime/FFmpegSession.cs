...
namespace FFmpegOut
{
    public sealed class FFmpegSession : System.IDisposable
    {
        ...
        )
        {
            ...
            var path = name + preset.GetSuffix();
        ...
        {
            // See: https://superuser.com/questions/1280255/vp8-single-pass-conversion-with-good-quality-in-ffmpeg
            string cmd = "-y -f rawvideo -vcodec rawvideo -pixel_format rgba"
                + " -colorspace bt709"
        ...
        public void PushFrameDirect(Texture source) {
            if (_pipe == null) {
                return;
            }
            ...
            // RenderTexture가 존재하지 않으면 생성
            if (_temporary == null || _temporary.width != source.width || _temporary.height != source.height)
            {
                if (_temporary != null)
                {
                    RenderTexture.ReleaseTemporary(_temporary);
                }
                _temporary = RenderTexture.GetTemporary(source.width, source.height, 0, RenderTextureFormat.ARGB32);
            }
            
            // 셰이더를 사용하여 상하 반전된 이미지를 임시 RenderTexture에 렌더링
            Graphics.Blit(source, _temporary, _blitMaterial, 0);

            // AsyncGPUReadback 요청
            var req = AsyncGPUReadback.Request(_temporary);
            
            // 요청이 완료될 때까지 기다리지 않고 콜백을 사용
            ...
            // 요청된 데이터 가져오기
            NativeArray<byte> data = req.GetData<byte>(0);
